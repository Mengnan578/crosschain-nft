...
contract NFTPoolLockAndRelease is CCIPReceiver, OwnerIsCreator {
    ...
    constructor(address _router, address _link, address _nftAddress) CCIPReceiver(_router) {
        s_linkToken = IERC20(_link);
        nft = MyToken(_nftAddress);
    }
    ...
    function lockAndSendNFT(
        ...
        address _receiver) public returns (bytes32) {
            // transfer NFT to this address to lock the NFT
            nft.transferFrom(msg.sender, address(this), _tokenId);
            bytes memory payload = abi.encode(_tokenId, _newOwner);
            bytes32 messageId = sendMessagePayLINK(_chainSelector, _receiver, payload);
            return messageId;
    }
